...
namespace Sinfonia.Implementations.ScoreDocument
{
    internal class ScoreLayoutDictionary : IScoreDocumentLayout
    {
        ...
        public void Apply(ScoreDocumentEditorProxy scoreDocumentCore, ScoreDocumentLayout layout)
        {
            ...
            var command = new SimpleCommand(() => { documentLayout = layout; }, () => { documentLayout = oldLayout; }).ThenInvalidate(notifyEntityChanged, scoreDocumentCore);
        ...
        {
            var transaction = commandManager.ThrowIfNoTransactionOpen();
            var command = new LayoutCommand<PageLayout>(pageLayoutDictionary, page.Guid, layout).ThenInvalidate(notifyEntityChanged, page.HostScoreDocument.Proxy());
            transaction.Enqueue(command);
        }
        ...
        {
            var transaction = commandManager.ThrowIfNoTransactionOpen();
            var command = new LayoutCommand<NoteLayout>(noteLayoutDictionary, note.Guid, layout).ThenInvalidate(notifyEntityChanged, note.HostMeasure.Proxy());
            transaction.Enqueue(command);
        }
        public void Restore(NoteEditorProxy element)
        {
            var transaction = commandManager.ThrowIfNoTransactionOpen();
            var command = new RemoveLayoutCommand<NoteLayout>(noteLayoutDictionary, element.Guid).ThenInvalidate(notifyEntityChanged, element.HostMeasure.Proxy());
            transaction.Enqueue(command);
        }
        ...
        {
            var transaction = commandManager.ThrowIfNoTransactionOpen();
            var command = new LayoutCommand<ChordLayout>(chordLayoutDictionary, chord.Guid, layout).ThenInvalidate(notifyEntityChanged, chord.HostMeasure.Proxy());
            transaction.Enqueue(command);
        }
        public void Restore(ChordEditorProxy element)
        {
            var transaction = commandManager.ThrowIfNoTransactionOpen();
            var command = new RemoveLayoutCommand<ChordLayout>(chordLayoutDictionary, element.Guid).ThenInvalidate(notifyEntityChanged, element.HostMeasure.Proxy());
            transaction.Enqueue(command);
        }
        ...
        public ScoreDocumentLayout DocumentLayout()
        {
        ...
        {
            if (pageLayoutDictionary.TryGetValue(element.Guid, out var value))
            {
                return value.Copy();
            }
